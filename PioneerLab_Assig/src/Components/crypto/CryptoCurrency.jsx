import React, { useState, useEffect } from 'react';
import './crypto.css'

const CryptocurrencyCard = ({ currency, symbol }) => {
  const [price, setPrice] = useState(null);

  useEffect(() => {
    const fetchCryptocurrencyPrice = async () => {
      try {
        const response = await fetch('https://api.coindesk.com/v1/bpi/currentprice.json');
        const data = await response.json();
        setPrice(data.bpi[currency].rate);
      } catch (error) {
        console.error(`Error fetching ${currency} price:`, error);
      }
    };

    fetchCryptocurrencyPrice();
  }, [currency]);

  return (
    <div className="cryptocurrency-card">
      <h3>Bitcoin Price ({currency})</h3>
      <p>{price ? `${symbol} ${price}` : 'Loading...'}</p>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '10px' }}>
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-info-circle" viewBox="0 0 16 16">
          <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16" />
          <path d="m8.93 6.588-2.29.287-.082.38.45.083c.294.07.352.176.288.469l-.738 3.468c-.194.897.105 1.319.808 1.319.545 0 1.178-.252 1.465-.598l.088-.416c-.2.176-.492.246-.686.246-.275 0-.375-.193-.304-.533zM9 4.5a1 1 0 1 1-2 0 1 1 0 0 1 2 0" />
        </svg>
        <button className='button'>Trade</button>
      </div>
    </div>
  );
};

const CryptocurrencyCards = () => {
  return (
    <div className="cryptocurrency-cards">
      <CryptocurrencyCard currency="USD" symbol="$" />
      <CryptocurrencyCard currency="GBP" symbol="£" />
      <CryptocurrencyCard currency="EUR" symbol="€" />
    </div>
  );
};

export default CryptocurrencyCards;
